...
Se o sensor for desconectado ou apresentar curto-circuito --- situação
detectada por leituras ADC nos limites do conversor ---, o sistema
identifica a falha, registra \textit{sensorOk} como inativo no monitor
serial e aplica um desconto no índice de confiabilidade da análise. O
cálculo de risco prossegue com os demais componentes disponíveis, conforme ilustram as \autoref{fig:serial_sensor_falha_pt1} e \autoref{fig:serial_sensor_falha_pt2}. Quando
o administrador acessa o comando de simulação via monitor serial
(\texttt{alerta amarelo} ou \texttt{alerta vermelho}, por exemplo), o LED correspondente ao nível
forçado acende imediatamente e o intervalo de envio ao \textit{backend}
assume o valor do nível simulado; na próxima leitura real do sensor, o
sistema restaura automaticamente o nível calculado pelo modelo, conforme ilustram a \autoref{fig:serial_modo_manual} e a \autoref{fig:serial_modo_manual_vermelho}.
...
BNDMET adiada''. Dados BNDMET disponíveis
mas com qualidade inferior a 80\% também geram um desconto adicional de
10 pontos na confiabilidade, porém são utilizados normalmente no modelo.
Se a precipitação horária I175 não registrar medições para o horário
atual, o valor de chuva atual é assumido como zero, sem prejuízo às
demais variáveis.
...
Em situações de falha persistente de conectividade Wi-Fi, o dispositivo
tenta reconectar automaticamente a cada 30 segundos e, após exceder o
limite de tentativas sem sucesso, reinicia de forma controlada para
garantir que o sistema retorne a um estado limpo. O mesmo mecanismo
se aplica a falhas repetidas de envio ao \textit{backend}: após o limite
ser atingido, a conexão Wi-Fi é reestabelecida do zero, conforme ilustra a \autoref{fig:serial_wifi_falha_envio}. Quando a memória
disponível do dispositivo cai abaixo de 5.000 bytes, o sistema reinicia
automaticamente para prevenir travamentos por esgotamento de \textit{heap}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/cap5_serial_wifi_falha_envio.png}
    \caption{Monitor serial durante falha persistente de Wi-Fi no envio ao \textit{backend}: tentativas de reconexão automática a cada 30 segundos, reconexão forçada do módulo Wi-Fi após exceder o limite de falhas consecutivas.}
    \label{fig:serial_wifi_falha_envio}
    \fonte{Elaborado pela autora.}
\end{figure}

Quando o \textit{backend} para de responder mesmo com Wi-Fi ativo, o comportamento observado é distinto: o contador de falhas de API é incrementado até acionar a reconexão forçada, conforme ilustra a \autoref{fig:serial_backend_sem_resposta}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/cap5_serial_backend_sem_resposta.png}
    \caption{Monitor serial com \textit{backend} sem resposta e Wi-Fi ativo: contador de falhas de API incrementado ciclo a ciclo e reconexão forçada do Wi-Fi ao atingir o limite de tentativas.}
    \label{fig:serial_backend_sem_resposta}
    \fonte{Elaborado pela autora.}
\end{figure}

O terceiro gatilho do \textit{watchdog} ocorre quando a memória disponível cai abaixo do limite seguro, resultando em reinicialização automática, conforme demonstra a \autoref{fig:serial_heap_critico}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/cap5_serial_heap_critico.png}
    \caption{Monitor serial com \textit{heap} crítico detectado: memória livre abaixo de 5.000 bytes, alerta registrado e reinicialização automática do dispositivo para recuperar o estado limpo.}
...
A próxima leitura real do sensor desfaz a simulação automaticamente.
Quando um comando não reconhecido é digitado, o sistema responde com
``Comando não reconhecido. Digite \texttt{help}'', conforme ilustra a \autoref{fig:serial_comando_nao_reconhecido}.
...
O \textit{header} da aplicação exibe o status ``Online'' enquanto o
sistema recebe leituras dentro do intervalo esperado. Esse indicador
...
Quando o sensor para de enviar dados por mais de 10 minutos, o indicador
no \textit{header} passa a exibir ``Degradado'' em laranja, conforme ilustra a \autoref{fig:dashboard_degradado}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/cap5_dashboard_degradado.png}
    \caption{\textit{Header} do \textit{dashboard} com status ``Degradado'' em laranja: sensor sem envio de dados há mais de 10 minutos; demais KPIs exibidos com os valores da última leitura disponível.}
    \label{fig:dashboard_degradado}
    \fonte{Elaborado pela autora.}
\end{figure}

Quando a própria API local fica inacessível, o \textit{backend} retorna erro de saúde e o indicador passa ao estado mais grave - muda para ``Offline'' em vermelho, conforme mostra a \autoref{fig:dashboard_indisponivel}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/cap5_dashboard_indisponivel.png}
    \caption{\textit{Header} do \textit{dashboard} com status ``Offline'' em vermelho: banco de dados inacessível, \textit{backend} retornando HTTP 503 na rota de saúde.}
    \label{fig:dashboard_indisponivel}
    \fonte{Elaborado pela autora.}
\end{figure}

Falhas ao carregar as estatísticas fazem com que os KPIs sejam exibidos com valor zero
enquanto o sistema aguarda nova tentativa, conforme ilustra a \autoref{fig:dashboard_kpis_zerados}.
...
Três botões de período fixo --- ``Últimas 24h'', ``7 dias'' e ``30 dias''
--- permitem alternar o intervalo de análise com um clique. O botão
ativo fica destacado em azul e a tabela é recarregada com os dados do
novo período, com KPIs recalculados, conforme ilustra a \autoref{fig:sensores_periodo_fixo}. Para intervalos específicos, o
...
O botão ``CSV'' gera um arquivo com BOM UTF-8 para
compatibilidade com o Microsoft Excel, contendo todas as leituras do
período selecionado com 41 colunas. O nome do arquivo inclui a data do
dia da exportação. As \autoref{fig:sensores_csv_pt1},
\autoref{fig:sensores_csv_pt2} e \autoref{fig:sensores_csv_pt3} exibem
o arquivo aberto no Excel, com os cabeçalhos das colunas identificados.
Quando o conjunto de dados filtrado está vazio, o botão de exportação é
exibido desabilitado, conforme ilustra a
\autoref{fig:sensores_csv_desabilitado}.
...
com os quatro KPIs de contagem por categoria no topo --- Total, Crítico,
Atenção e Ruptura --- seguidos da lista
de alertas ordenada do mais recente para o mais antigo, com paginação
a 10 registros por página.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/cap5_alertas_lista.png}
    \caption{Central de Alertas: KPIs de contagem por categoria e lista paginada de registros.}
...
Cada card de alerta é expansível e, além dos quatro blocos de detalhe
...
é fixada pelo hardware e não sofre descontos. As
\autoref{fig:alertas_expandido_pt1}, \autoref{fig:alertas_expandido_pt2}
e \autoref{fig:alertas_expandido_pt3} exibem os alertas dos níveis Vermelho, Amarelo e Ruptura, respectivamente.
...
em \texttt{logs\_alertas}, conforme ilustra a
\autoref{fig:email_recebido_pt5} e contabilizadas no \textit{toast} de confirmação
exibido ao administrador, conforme ilustra a
\autoref{fig:alertas_entrega_parcial_pt2}, sem interromper os demais envios do lote. Falhas
assíncronas, como rejeições por domínio inexistente (\textit{bounce}),
ocorrem após o servidor SMTP aceitar a mensagem: o \textit{backend}
registra o envio como bem-sucedido e a notificação de falha retorna
posteriormente à caixa do remetente, conforme ilustra a
\autoref{fig:email_recebido_pt6}, fora do controle do sistema. Esse
comportamento é inerente ao protocolo SMTP e exigiria integração com
serviço especializado de monitoramento de bounces para tratamento
automatizado.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{figures/cap5_email_recebido_pt5.png}
    \caption{Detalhe no banco de dados (parte~5):
...
A tela de Relatórios carrega automaticamente, ao ser aberta, um relatório
referente aos últimos sete dias. O administrador pode redefinir o período
pelos campos de data de início e fim e clicar em ``Gerar Relatório'' para
atualizar a análise. As \autoref{fig:relatorio_gerado_pt1} a
\autoref{fig:relatorio_gerado_pt6} exibem a tela com o relatório gerado
para um período customizado.
...
Ao alternar para a aba ``Administradores'', a lista é recarregada e as 
opções de exportação refletem o novo contexto. Filtros e termos
de busca são limpos automaticamente na troca de aba. Quando a busca não
encontra resultado, o sistema exibe o \textit{toast} ``Erro ao carregar usuários'', conforme ilustra a \autoref{fig:usuarios_filtro_sem_resultado}. 
...
ao risco &
Protótipo montado e em operação; LEDs acendem no nível correspondente; \textit{buzzer} soa com frequência e cadência crescentes conforme a gravidade do índice (80--84\%, 85--89\% e $\geq$90\%) \\
\hline
b & Integração com BNDMET/DECEA e OpenWeatherMap para precipitação
histórica e previsão &
Ambas as APIs integradas e em operação; acumulados de 24h, 7d e 30d via BNDMET e previsão de 24h via OWM; proteção pré-NTP e \textit{retry} acelerado implementados \\
\hline
c & Modelo de sete variáveis ponderadas com mecanismo de amplificação
$\times 1{,}20$ &
Modelo em operação; todos os sete componentes calculados e exibidos; amplificação registrada no monitor serial e no \textit{dashboard}; protocolo de ruptura com \textit{cooldown} de três leituras validado \\
\hline
d & \textit{Backend} Node.js com banco de séries temporais e API
RESTful &
\textit{Backend} em operação com PostgreSQL e TimescaleDB; dados recebidos e persistidos sem recálculo; 41 campos por leitura; autenticação JWT funcional \\
\hline
e & Interface \textit{web} com histórico, alertas, meteorologia e
gestão de usuários &
Seis telas implementadas e funcionais: \textit{dashboard}, Sensores, Alertas, Relatórios, Usuários e Configurações; todos os fluxos de sucesso e alternativos validados \\
\hline
f & Sistema de envio de alertas por e-mail acionado pelo administrador &
...
e-mail HTML colorido por criticidade entregue aos destinatários \\
\hline
\end{tabular}
\fonte{Elaborado pela autora.}
\end{table}

Sobre o objetivo específico~f), registra-se que a especificação original
previa envio automático de alertas. O sistema implementado adota envio
manual acionado pelo administrador, decisão tomada durante o
desenvolvimento por razões de controle operacional: em cenários reais,
o disparo automático de alertas de evacuação sem validação humana pode
gerar falsos alarmes com consequências sociais significativas. O
administrador analisa o contexto completo --- incluindo os dados de
confiabilidade e os componentes individuais do modelo --- antes de
decidir pelo acionamento da notificação.
...
\cite{quispe2018instrumentacao}. Isso demonstra que tecnologias de
...
ausente nos sistemas manuais que predominavam em Mariana e Brumadinho
no momento dos acidentes \cite{taliberti2025tragedia}.

A abordagem multidimensional do modelo de risco tem respaldo direto
na literatura. \citeonline{rico2008} identificaram que 39\% dos
rompimentos documentados resultaram da combinação de múltiplos fatores,
não de uma causa isolada, o que justifica a integração de variáveis
...
--- correspondente a 40\% do índice --- está alinhado à preponderância
de causas de saturação do maciço nos acidentes documentados
...
O índice de confiabilidade auditável diferencia o sistema desenvolvido
da maioria das soluções de baixo custo descritas na literatura, que
emitem alertas sem qualificar a incerteza dos dados subjacentes. A
capacidade de o administrador consultar, para qualquer leitura histórica,
quais fontes estavam indisponíveis e qual o impacto quantitativo dessa
indisponibilidade está alinhada às recomendações do ICOLD sobre
transparência em sistemas de monitoramento \cite{icold2022tailings}.

Três limitações merecem destaque. A primeira diz respeito ao sensor
higrômetro resistivo FC-28: por ser um componente voltado a aplicações
educacionais, apresenta precisão e durabilidade inferiores às de
instrumentos geotécnicos certificados. A eletrólise acelerada nos
eletrodos em solo permanentemente úmido compromete a longevidade da
sonda em instalações de longa duração. Em uma implantação real, o
...
A segunda limitação refere-se à dependência de conectividade Wi-Fi
e de APIs externas com disponibilidade variável. Embora o sistema
implemente \textit{retry} automático e o modelo de confiabilidade
penalize indisponibilidades, a ausência prolongada dos dados BNDMET
suprime três dos sete componentes do índice de risco, reduzindo
substancialmente sua acurácia. Em instalações reais, conectividade
redundante --- Wi-Fi associada a módulo 4G --- e uma estação
pluviométrica local minimizariam essa dependência.

A terceira limitação refere-se ao escopo de validação: o sistema foi
testado em ambiente controlado de bancada, sem implantação \textit{in
loco} em uma barragem real. A representatividade dos dados pluviométricos
da estação D6594 para um ponto específico de monitoramento, a localização
ideal da sonda no perfil do maciço e a calibração dos limiares para
diferentes tipos de rejeito demandariam validação geotécnica
especializada antes de uma aplicação operacional.

Em relação ao TCC de referência desta instituição --- o Med Alarm, de
\citeonline{larman2007utilizando} --- o presente trabalho apresenta maior
complexidade de integração, reunindo três camadas funcionais, duas APIs
externas, banco de dados de séries temporais e modelo matemático
embarcado em um único sistema. A decisão de processar o cálculo de risco
integralmente no microcontrolador garante análise contínua mesmo em
cenários de conectividade intermitente, propriedade crítica para sistemas
destinados a ambientes industriais remotos onde a infraestrutura de rede
pode ser instável.