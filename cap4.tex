...
\hline
$0{,}00 \leq F_{risco} \leq 0{,}50$ & Verde   & Condições normais de operação           \\
$0{,}51 \leq F_{risco} \leq 0{,}80$ & Amarelo & Atenção — monitoramento intensificado   \\
$F_{risco} > 0{,}80$                 & Vermelho & Alerta crítico — ação imediata requerida \\
\hline
\end{tabular}
\fonte{Elaborado pela autora.}
\end{table}
...
\begin{enumerate}
  ...
  \item O sistema classifica o nível por faixa: VERDE
        ($\leq 0{,}50$), AMARELO ($\leq 0{,}80$) ou VERMELHO
        ($> 0{,}80$).
...
\textbf{Frequência:} a cada 30 minutos em operação normal; a
cada 30 segundos em modo de \textit{retry} após falha.
...
\textbf{Cenário de Sucesso Principal (Fluxo Básico):}
\begin{enumerate}
  \item O Administrador acessa \texttt{/dashboard}.
  \item O sistema exibe os KPIs: umidade atual, fator de
        ...
        da análise.
  \item O \textit{header} inicia dois intervalos:
        \texttt{checkHealth()} a cada 30 segundos via
        \path{GET /api/health} e
        \texttt{loadNotifications()} a cada 60 segundos
        via \path{GET /api/sensor/alertas?limite=10}.
  \item O sistema exibe o status do sistema no
        \textit{header} como ``Online'' (verde),
        ``Degradado'' (laranja) ou ``Offline''
        (vermelho) com base no campo \texttt{status}
        retornado pelo \texttt{/health}.
  \item O sino exibe \textit{badge} apenas para alertas
        com \texttt{nivelAlerta = VERMELHO} ou
        \texttt{indiceRisco = 100} das últimas 24 horas.
\end{enumerate}
...
\begin{enumerate}
  \renewcommand{\theenumi}{5a}
  \renewcommand{\labelenumi}{\theenumi.}
  \item Sensor \textit{offline} detectado no passo 5
  \begin{enumerate}
  \item Quando \texttt{minutosUltimaLeitura} $\geq 10$,
        o \texttt{/health} retorna
        \texttt{sensor: ``offline''}.
...
\begin{enumerate}
  ...
  \begin{enumerate}
  \item O \texttt{/health} retorna
        \texttt{status: ``unhealthy''} com HTTP 503.
  \item O \textit{header} exibe status ``Indisponível''.
  \end{enumerate}
\end{enumerate}

\textbf{Pós-Condições:} \textit{dashboard} exibido com dados
atualizados; intervalos de \textit{polling} ativos enquanto
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC28 --- Gerenciar usuários básicos}\hfill\break
...
\begin{enumerate}
  \renewcommand{\theenumi}{4a}
  \renewcommand{\labelenumi}{\theenumi.}
  \item Criação de usuário (UC29) no passo 4
  \begin{enumerate}
  \item O sistema executa o fluxo do UC29.
...
\begin{enumerate}
  \renewcommand{\theenumi}{4b}
  \renewcommand{\labelenumi}{\theenumi.}
  \item Edição de usuário (UC30) no passo 4
  \begin{enumerate}
...
\begin{enumerate}
  ...
  \renewcommand{\labelenumi}{\theenumi.}
  \item Desativação ou reativação (UC31) no passo 4
  \begin{enumerate}
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC29 --- Cadastrar usuário básico (pelo Administrador)}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC30 --- Editar usuário básico}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC31 --- Ativar / Desativar usuário}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC32 --- Filtrar usuários por nome ou e-mail}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC33 --- Alternar aba de usuários}\hfill\break
...
\begin{enumerate}
  ...
  \item O sistema atualiza a tabela, os KPIs e o contexto
        de exportação (UC34) para refletir o tipo de
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC34 --- Exportar usuários em CSV}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC35 --- Gerenciar administradores}\hfill\break
...
% ─────────────────────────────────────────────────────────────
\paragraph{UC36 --- Cadastrar administrador}\hfill\break
...
alerta vigente. Os intervalos de envio para o \textit{backend} são adaptativos:
60 segundos no nível VERDE, 20 segundos no nível AMARELO e 10 segundos no
nível VERMELHO, priorizando a frequência de atualização nos cenários de maior
criticidade.
